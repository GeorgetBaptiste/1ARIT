...
class ImageManipulation:
    ...
    def __init__(self, screen_width, screen_height):
        ...
        self.__button_create_key_p = (screen_width*0.5, screen_height*0.4)
        self.__button_chiffrer_p = (screen_width*0.5, screen_height*0.6)
        self.__button_dechiffrer_p = (screen_width*0.5, screen_height*0.8)
        self.__entry_p = (screen_width*0.5, screen_height*0.2)
        self.__button_masquer_image_p = (screen_width*0.5, screen_height*0.4)
        self.__button_demasquer_image_p = (screen_width*0.5, screen_height*0.6)
        self.__button_masquer_texte_p = (screen_width*0.5, screen_height*0.4)
        self.__button_demasquer_texte_p = (screen_width*0.5, screen_height*0.6)
        self.__progressbar_p = (screen_width*0.5, screen_height*0.9)
                
        self.__page_masquage_texte = [[self.__button_back, self.__entry, self.__button_masquer_texte, self.__button_demasquer_texte, self.__progressbar], [self.__button_back_p, self.__entry_p, self.__button_masquer_texte_p, self.__button_demasquer_texte_p, self.__progressbar_p]]
        self.__page_masquage_image = [[self.__button_back, self.__entry, self.__button_masquer_image, self.__button_demasquer_image, self.__progressbar], [self.__button_back_p, self.__entry_p, self.__button_masquer_image_p, self.__button_demasquer_image_p, self.__progressbar_p]]
        self.__page_chiffrement_image = [[self.__button_back, self.__button_chiffrer, self.__button_dechiffrer, self.__progressbar, self.__button_create_key, self.__entry], [self.__button_back_p, self.__button_chiffrer_p, self.__button_dechiffrer_p, self.__progressbar_p, self.__button_create_key_p, self.__entry_p]]
        ...
        if width_image == width_key_image and height_image == height_key_image:
            ...
            for x in range(width_image):
                for y in range(height_image):
                    ...
                    pixel_count += 1
                    if round(pixel_count*100/(width_image*height_image)) > save:
                        save = round(pixel_count*100/(width_image*height_image))
            ...
            for y in range(height):
                for x in range(width):
                    ...
                    for z in range(len(rgb)):
                        if pixel_count < len(chain_bin):
                            rgb[z] += int(self.str_to_bin(chr(rgb[z]))[-1])^int(chain_bin[pixel_count])
                        pixel_count += 1
                        if round(pixel_count*100/(width*height)) > save:
                            save = round(pixel_count*100/(width*height))
                            self.__progressbar['value'] = save
